	...
	sort_times_t sort_times)
{
	std::cout << sort_function_name << "\n"
		<< "\tAverage Case:\t" << sort_times.average_case_seconds
		<< " seconds\n"
		<< "\tBest Case:\t" << sort_times.best_case_seconds << " seconds\n"
		<< "\tWorst Case:\t" << sort_times.worst_case_seconds
		<< " seconds\n\n";
}
...
double run_sort_function(void sort_function(int[], const int), int array[],
	const int length)
{
	...
	const int array_length)
{
	...
	if (!array)
	{
		std::cout << "Error: wasn't able to allocate memory for "
			<< get_sort_function_name(sort_function) << "'s int array.\n"
			<< "Exiting..."
			<< std::endl;
	...
	for (int i = 0; i < array_length; i++)
		array[i] = std::rand();
	sort_times.average_case_seconds =
		run_sort_function(sort_function, array, array_length);

	// best case
	for (int i = 0; i < array_length; i++)
		array[i] = i;
	sort_times.best_case_seconds =
		run_sort_function(sort_function, array, array_length);

	// worst case
	for (int i = 0; i < array_length; i++)
		array[i] = array_length - i;
	sort_times.worst_case_seconds =
		run_sort_function(sort_function, array, array_length);
